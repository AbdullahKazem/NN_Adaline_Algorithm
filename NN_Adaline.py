...
def trainAdaline(weight, data, epochs, lr, mseThreshold):
    check = 0
    for epoch in range(epochs):
        ...
        for i in range(len(data)):
            trainEx = data.iloc[i, :3].to_numpy()
            y_actual = data.iloc[i, -1]
            net = calculateNetValue(trainEx, weight)
            error = y_actual - net
            errorlist.append(error)
        sum_squar_error = sum(np.square(errorlist))
        print("sum squre error: ", sum_squar_error)
        MSE = (1/len(data))*(sum_squar_error*0.5)
        print("MSE:", MSE)
        if MSE < mseThreshold:
            check = 1
            print("MSE is less than threshold and match stoping criteria")
            break
    if check == 0:
        print("MSE didn't match stopping criteria after all epocs")
...
def RunWholeProgram():
    ...
    #train dataset
    actualWeight = trainAdaline(weight, trained_data, user_epochs, user_lr, user_threshold)
